...
const GalleryCard: React.FC<GalleryCardProps> = ({ title, description, date }) => {
    return (    
        <div className="flex flex-col w-75 h-[19vw] bg-[#FCFCFC] rounded-[20px] p-4 border-4 border-white hover:cursor-pointer">
            ...
            <div className="flex flex-col gap-3 text-left h-full relative">
                {/* <h3 className="gallery-card__title">{title}</h3>*/}
                <p className="gallery-card__description">{description}</p> 
                <div className='absolute bottom-0 right-0'>
                    <button className="rounded-full w-8 h-8 bg-[#C9DEFF] flex justify-center items-center text-center"></button>
                </div>
            </div>
        </div>
    );
